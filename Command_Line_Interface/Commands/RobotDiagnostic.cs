...
using Mir_Utilities.RosTools;
using Mir_Utilities.RosTools.Schema;

namespace Console_Input.Commands;

public class RobotDiagnostic
{
    /*
    Command to clear all footprints of the credentials provided from a robot
    Usage: diagnose <robot(s)>
    Alias: -
    Arguments:
    ...
    */
    ...
    public void CommandHandler(List<string> arguments, List<string> flags, List<string> options)
    {
        ...
        foreach (var robot in targetRobotsInformation)
        {
            try
            {
                RobotHealth robotHealth = RosGetDiagnosisInfomation.GetRobotHealth(robot);
            }
            catch (Exception e)
            {
                Console.ForegroundColor = ConsoleColor.Red;
